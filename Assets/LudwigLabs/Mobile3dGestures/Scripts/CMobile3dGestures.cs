...
namespace Gestures
{
	...
	public class CMobile3dGestures : MonoBehaviour
	{
		...
		private Vector3				m_current;				// Current acceleration with gravity, in device's space
		private Vector3				m_user;					// User acceleration without gravity, in device's space
		private Spike				m_cur;
		...
		public bool Resume()
		{
			...
			if (m_running)
			{
				...
				if (m_hasGyro)
				{
					m_gravity	= Input.gyro.gravity;
					m_current	= Input.acceleration;
					m_user		= Input.gyro.userAcceleration;
				}
				else
				{
					m_gravity	= Input.acceleration;
					m_current	= Input.acceleration;
					m_user		= Vector3.zero;
				}
			}
		...
		private void FixedUpdate()
		{
			...
			{
				m_gravity			= Vector3.Lerp(m_gravity, Input.acceleration, c_gravityFilter);
				m_current			= Input.acceleration;
